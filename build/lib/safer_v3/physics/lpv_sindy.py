...
class LPVSINDyMonitor(nn.Module):
    ...
    def __init__(
        ...
        # Function library - use LPVAugmentedLibrary if adaptive scheduling is enabled
        if library is None:
            if self.config.use_adaptive_scheduling:
                # Use LPV augmented library with health-dependent terms
                from safer_v3.physics.library import LPVAugmentedLibrary
                self.library = LPVAugmentedLibrary(
                    degree=self.config.polynomial_degree,
                    include_bias=True,
                    include_interaction=self.config.include_interactions,
                )
                logger.info("Using LPVAugmentedLibrary with health-dependent scheduling")
            else:
                # Use standard polynomial library
        ...
        # Store scheduling parameter for later use
        self._scheduling_param = None
    ...
    def fit(
        ...
        When use_adaptive_scheduling is enabled, this method will:
        1. Auto-compute scheduling parameter p(t) from sensor data
        2. Use LPVAugmentedLibrary with p-dependent basis functions
        3. Learn health-dependent dynamics coefficients
        ...
        # Compute scheduling parameter if adaptive scheduling is enabled
        p_train = None
        p_val = None
        if self.config.use_adaptive_scheduling:
            p_full = self.compute_scheduling_parameter(
                X,
                nominal_egtm=self.config.nominal_egtm,
                min_egtm=self.config.min_egtm,
            )
            if validate:
                p_train = p_full[:n_train]
                p_val = p_full[n_train:]
            else:
                p_train = p_full
            
            # Store for later use
            self._scheduling_param = p_train
            
            logger.info(
                f"Computed scheduling parameter: "
                f"min={p_train.min():.4f}, max={p_train.max():.4f}, "
                f"mean={p_train.mean():.4f}"
            )
        ...
        # Transform to library features - pass scheduling parameter if using LPV
        if self.config.use_adaptive_scheduling and p_train is not None:
            # LPVAugmentedLibrary.transform accepts scheduling parameter
            Theta_train = self.library.transform(X_train, p_train)
        else:
        ...
        train_residuals = self._compute_residuals(X_train, p_train)
        ...
        results = {
            ...
            'use_adaptive_scheduling': self.config.use_adaptive_scheduling,
        }
        
        if self.config.use_adaptive_scheduling and p_train is not None:
            results['scheduling_param_stats'] = {
                'min': float(p_train.min()),
                'max': float(p_train.max()),
                'mean': float(p_train.mean()),
                'std': float(p_train.std()),
            }
        
        # Validation metrics
        if validate and X_val is not None and len(X_val) > self.config.window_size:
            val_residuals = self._compute_residuals(X_val, p_val)
    ...
    def _compute_residuals(
        ...
        Args:
            X: State trajectory, shape (n_samples, n_features)
            p: Optional scheduling parameter, shape (n_samples,)
               Required when use_adaptive_scheduling is enabled.
        ...
        # Transform - pass scheduling parameter if using LPV
        if self.config.use_adaptive_scheduling and p is not None:
            Theta = self.library.transform(X, p)
        else:
